...
export default function AnimatedHeroTitle({ children, className = "" }: AnimatedHeroTitleProps) {
  ...
  return (
    <div className="relative inline-block">
      ...
      >
        {/* Animated text with typewriter effect letter-by-letter */}
        <motion.div animate={controls}>
          {words.map((word) => (
            <span key={`word-${word}`} className="inline-block mr-2">
              {word.split("").map((letter, letterIndex) => {
                const wordIndex = words.indexOf(word);
                const globalIndex = words.slice(0, wordIndex).join("").length + wordIndex + letterIndex;
                return (
                  <motion.span
                    key={`letter-${word}-${letter}-${globalIndex}`}
                    ...
                    whileHover={{
                      y: -5,
                      textShadow: "0 5px 15px rgba(59, 130, 246, 0.4)",
          ...
          <motion.span
            ...
            transition={{
              duration: 1,
              repeat: Number.POSITIVE_INFINITY,
              delay: 0.5 + text.length * 0.05 + 0.5,
            }}
          />
        </motion.div>
        ...
        <motion.div
          ...
          animate={{
            ...
            boxShadow: isHovered ? "0 0 20px rgba(59, 130, 246, 0.6)" : "0 0 10px rgba(59, 130, 246, 0.3)",
          }}
          transition={{
            duration: 0.6,
            delay: 1.5,
