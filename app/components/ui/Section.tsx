...
export function Section({
  ...
  return (
    <section id={id} className={clsx(bgClasses[background], paddingClasses[padding], className)}>
      {children}
    </section>
  );
}
...
export function SectionHeader({ title, subtitle, centered = true, className = "" }: SectionHeaderProps) {
  return (
    ...
    >
      <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 text-gray-900">{title}</h2>
